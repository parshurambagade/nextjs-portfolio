...
const FeaturedProjects = () => {
  ...
  return (
    <div
      id="projects"
      className="min-h-screen w-full bg-gray-900/[.5] dark:bg-gray-900/[.5]  py-4 md:py-12  px-2 md:px-12 lg:px-20"
    >
...
const ProjectCard = ({ project }: { project: Project }) => {
  return (
    <div className="w-full md:w-3/4 lg:w-[24rem] h-full">
      <BackgroundGradient className="rounded-[22px] h-full max-w-full  p-4 py-6 sm:p-6 bg-zinc-900 dark:bg-zinc-900">
        ...
        <div className="my-4">
          ...
          <div className="flex flex-col gap-2">
            {project.desc.map((line: string, i: number) => (
              <p
                key={i}
                className="list-item  ml-6 md:ml-3 text-sm text-neutral-400 dark:text-neutral-400"
              >
                {line}
              </p>
            ))}
          </div>
          <div className="flex gap-2  flex-wrap my-4 text-gray-400 text-xs">
            {project.techstack.map((tech: string, i: number) => (
              <p
                key={i}
                className="border border-gray-700 bg-slate-800/[.5] py-1 px-2 rounded-xl"
              >
                {tech}
              </p>
            ))}
        ...
        <div className="flex justify-evenly md:justify-between mt-6 ">
          <Link
            ...
            target="_blank"
            className="flex text-sm items-center underline underline-offset-4  font-normal pr-4 py-2 bg-transparent text-gray-400 hover:text-cyan-200 rounded-lg transform hover:-translate-y-1 transition duration-400"
          >
            <span className="mr-[.3rem]">Code</span>
            <FaGithub />
          </Link>
          <Link
